...
class DatasetGenerator(Node):
    ...
    def camera_depth_data_save_to_file(self):
        bridge = CvBridge()
        file_name = "{}_{}.png".format(self.save_file_name,self.sample_idx)


        depth_msg = self.camera_depth_msg_latest
        converted_image = bridge.imgmsg_to_cv2(depth_msg, desired_encoding="passthrough")
        depth_path = os.path.join(self.dataset_path, self.camera_depth_data_folder, file_name)
